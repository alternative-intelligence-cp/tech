...
- 🤖 **Autonomous Processing**: LLM-powered classification, validation, and relationship extraction
- 🔍 **Hybrid Search**: Combines vector similarity (semantic) with BM25 (lexical) using Reciprocal Rank Fusion
- 📊 **Knowledge Graph**: PostgreSQL-based semantic graph with cycle detection
- 🗜️ **Auto-Archive**: Compresses and removes processed files to keep repository clean
- ⚡ **Async Pipeline**: BullMQ-based queue prevents blocking, handles retries gracefully
- 🎯 **Codebase-Aware**: Custom text search dictionary preserves exact technical terminology

## Architecture

```
File → Watcher → Queue → [Extractor → Classifier → Checker → Function Caller → Embedder] → Database → Archive
```
...
Formula: `RRF_score(d) = Σ(1 / (60 + rank_i))` for each ranker `i`
...
# (extraction → classification → validation → database → archive)
...
1. Nikola Consciousness Field Integration (Score: 0.0724, Match: both)
   ...
   ConsciousnessField C++ library with Nitpick's 6-stream I/O...
```
...
├── agents.js           # LLM pipeline (Classifier, Checker, Function Caller)
├── executor.js         # Database command execution
├── archiver.js         # ZIP compression
├── index.js            # Main BullMQ worker + file watcher
├── search.js           # Hybrid search CLI
├── package.json
└── README.md
```
